...
| 版 | 日付 | 変更内容 |
|---|---|---|
| 1.0 | 2026-07-18 | `causal-atelier` の当時の実装に合わせて全面改訂 |
...
本書は、Pythonパッケージとして実装されている `causal-atelier` を、Analysis-ready Datasetの登録、変数の意味付け、因果探索、探索結果の比較・選択、因果設計、因果推論、診断、再現性情報の管理を備えたWebサービスへ発展させるための要件を定義する。
...
1. causal-atelierは、汎用ETL基盤やData Warehouseの代替を目指さない。
...
causal-atelierは、これらを一つの分析来歴として結び付けることで、次の価値を提供する。
...
causal-atelierは、入力データを必ず物理コピーすることを将来要件としない。外部Datasetの不変snapshotを一意に参照でき、実行時に権限付きでmaterializeまたはqueryできればよい。
...
- `causal-atelier-discovery`
- `causal-atelier-inference`
- `causal-atelier-pipeline`
...
`src/causal_atelier/etl/completejourney/`および`src/causal_atelier/application/pipeline/etl.py`の既存class/functionを維持し、既存入力から既存の8つのParquet出力を生成できること。
...
- `causal_atelier.etl.completejourney.*` のETL class
- `causal_atelier.application.pipeline.etl.execute_completejourney_etl`
- `causal_atelier.preprocessing.*` の既存Discovery/Inference Feature Build
...
これはcausal-atelier packageからETL責務や前処理classを除去する判断ではない。CLI、既存Pipeline、デモ、テスト、Advanced利用における既存ETL・Feature Buildは維持する。
...
CSV/Parquet uploadは、causal-atelier単体でMVP価値を試すための簡易入力Adapterである。将来的な本番入力は不変の外部Dataset Referenceを想定する。
...
7. Databricks Job Run IDとcausal-atelier lineageの接続方法
...
- `frontend/app.js`
- `frontend/styles.css`
- `src/causal_atelier/interfaces/api/app.py`
- `src/causal_atelier/interfaces/api/schemas/resources.py`
- `src/causal_atelier/interfaces/api/routers/projects.py`
...
- `src/causal_atelier/etl/completejourney/load.py`
