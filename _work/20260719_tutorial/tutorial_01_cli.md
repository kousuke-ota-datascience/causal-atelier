# causal-atelier CLIチュートリアル

## 1. このチュートリアルで行うこと

causal-atelierのCLIには、次の3つの入口があります。

| コマンド | 用途 |
|---|---|
| `causal-atelier-discovery` | 因果探索だけを実行する |
| `causal-atelier-inference` | Edge WeightまたはTreatment Effect推定だけを実行する |
| `causal-atelier-pipeline` | 因果探索から因果推論までを一つの再現可能なRunとして実行する |

通常は、manifest生成とstage間の整合性検査を含む`causal-atelier-pipeline`を推奨します。
...
```bash
uv run causal-atelier-discovery --help
uv run causal-atelier-inference --help
uv run causal-atelier-pipeline --help
```
...
```bash
uv run causal-atelier-discovery \
  --project-root "$PWD" \
...
```bash
uv run causal-atelier-inference \
  --project-root "$PWD" \
