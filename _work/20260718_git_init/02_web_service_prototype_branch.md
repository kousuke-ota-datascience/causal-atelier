...
| Remote name | `causal-atelier` |
| Remote repository | `git@github.com:kousuke-ota-datascience/causal-atelier.git` |
| Base branch | `main` |
...
```bash
git switch main
git pull --ff-only causal-atelier main
```
...
```bash
git push -u causal-atelier prototype/web-service
```

`-u` により、ローカルブランチの追跡先として
`causal-atelier/prototype/web-service` が設定される。
...
```text
* prototype/web-service ... [causal-atelier/prototype/web-service]
```
...
- リモート名がまだ `origin` の場合は、コマンド中の `causal-atelier` を `origin` に
  読み替える。
