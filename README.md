...
`CAUSAL_ATELIER_AUTH_MODE=oidc` とし、issuer、audience、JWKS URLを設定して
...
```bash
uv sync --all-groups
export CAUSAL_ATELIER_AUTO_CREATE_SCHEMA=true
uv run causal-atelier-api
# 別terminal
uv run causal-atelier-worker
```
