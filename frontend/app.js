...
async function loadProject(projectId) {
  ...
  try {
    ...
    const [datasets, runs, members, configurations, graphs] = await Promise.all([
      ...
      loadResource("Runs", `/api/v1/executions?project_id=${encodedProjectId}&limit=200`, {items: []}),
...
function renderOverview() {
  ...
  const hasDiscovery = state.runs.some(run => run.execution_kind === "DISCOVERY" && run.status === "SUCCEEDED");
  const hasGraph = savedGraphs().length > 0;
  const hasInference = state.runs.some(run => run.execution_kind === "INFERENCE" && run.status === "SUCCEEDED");
  const journey = [
...
function runRow(run) {
  return `<div class="run-row"><div><b>${escapeHtml(run.execution_kind)} · ${shortId(run.id)}</b><small>${formatDateTime(run.submitted_at)}</small></div>${statusPill(run.status)}</div>`;
}
...
function renderRunTables() {
  renderRunTable("#discovery-runs", state.runs.filter(run => run.execution_kind === "DISCOVERY"), "discovery");
  renderRunTable("#inference-runs", state.runs.filter(run => run.execution_kind === "INFERENCE"), "inference");
}
...
function renderRunTable(selector, runs, kind) {
  $(selector).innerHTML = runs.length ? `<table class="data-table"><thead><tr><th>Run</th><th>Mode</th><th>Status</th><th>Submitted</th><th></th></tr></thead><tbody>${runs.map(run => `<tr>
    <td><strong>${shortId(run.id)}</strong><small>${escapeHtml(run.execution_kind)}</small></td><td>${escapeHtml(run.execution_mode)}</td>
...
function renderRunSelects() {
  const discoveryRuns = state.runs.filter(run => run.execution_kind === "DISCOVERY");
  const inferenceRuns = state.runs.filter(run => run.execution_kind === "INFERENCE");
...
async function createDiscoveryRun(event) {
  ...
  try {
    ...
    const run = await submitRun({
      execution_kind: "DISCOVERY", execution_mode: form.elements.execution_mode.value, random_seed: Number(form.elements.random_seed.value),
      stages: [{
      ...
      metadata: {ui: "ariadne", requested_algorithms: algorithms},
    });
...
async function createInferenceRun(event) {
  ...
  try {
    ...
    const run = await submitRun({
      execution_kind: "INFERENCE", execution_mode: form.elements.execution_mode.value, random_seed: Number(form.elements.random_seed.value),
      stages: [{
      ...
      }], metadata: {ui: "ariadne", causal_graph_version_id: graph.id},
    });
    upsertRun(run); renderOverview(); renderRunTables(); renderRunSelects();
...
async function submitRun(document) {
  return api("/api/v1/executions", {method: "POST", headers: {"Idempotency-Key": crypto.randomUUID()}, body: JSON.stringify({project_id: state.project.id, ...document})});
}
...
function showRunMonitor(kind, run) {
  ...
  panel.innerHTML = `<div class="run-monitor-header"><div><p class="kicker">${active ? "RUN IN PROGRESS" : "RUN STATUS"}</p><h3>${escapeHtml(run.execution_kind)} · ${shortId(run.id)}</h3><p>${escapeHtml(run.execution_mode)} · submitted ${formatDateTime(run.submitted_at)}</p></div>${statusPill(run.status)}</div>
...
function pollRun(runId, kind) {
  ...
  const check = async () => {
    try {
      const run = await api(`/api/v1/executions/${runId}`);
      upsertRun(run); showRunMonitor(kind, run); renderRunTables(); renderRunSelects(); renderOverview();
      if (TERMINAL_STATUSES.has(run.status)) {
        state.pollers.delete(key);
        toast(`${run.execution_kind} Runは ${run.status} で終了しました。`, run.status === "SUCCEEDED" ? "success" : "error");
...
async function loadDiscoveryFromRun(runId) {
  ...
  try {
    const summary = await api(`/api/v1/executions/${runId}/results`);
...
async function refreshResources() {
  const [datasets, runs, configurations, graphs] = await Promise.all([
    ...
    api(`/api/v1/executions?project_id=${state.project.id}&limit=200`),
