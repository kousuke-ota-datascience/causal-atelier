...
document.addEventListener('change',event=>{const select=event.target;if(select instanceof HTMLSelectElement&&select.classList.contains('datasets-select')){select.dataset.selectedDatasetVersionId=select.value;if(select.closest('#inference-form'))reconcileInferenceTreatment({announce:true})}});
...
function renderAnalysisViews(){
  ...
  target.innerHTML=state.analysisViews.length?`<table><thead><tr><th>Name / Version</th><th>Status</th><th>Rows</th><th>Hash</th><th>Operation</th></tr></thead><tbody>${state.analysisViews.map(view=>`<tr><td>${escapeHtml(view.name)} / v${view.version_number}</td><td><span class="status ${view.status}">${view.status}</span></td><td>${view.manifest.output_row_count??'—'}</td><td>${escapeHtml((view.content_hash||'draft').slice(0,12))}</td><td><button type="button" onclick="showAnalysisView('${view.analysis_view_id}')">表示</button>${view.status==='DRAFT'?` <button type="button" onclick="fixAnalysisView('${view.analysis_view_id}')">Validate &amp; FIX</button>`:''}</td></tr>`).join('')}</tbody></table>`:'Analysis Viewはありません';
...
window.showAnalysisView=async id=>{if(!state.project)return;try{const view=await api(`/projects/${state.project.project_id}/analysis-views/${id}`);$('#analysis-view-details-title').textContent=`${view.name} / v${view.version_number}`;$('#analysis-view-details').innerHTML=`<dl class="read-only-details"><dt>Status</dt><dd><span class="status ${escapeHtml(view.status)}">${escapeHtml(view.status)}</span></dd><dt>View key</dt><dd>${escapeHtml(view.view_key)}</dd><dt>Source Dataset Version</dt><dd>${escapeHtml(view.source_dataset_version_id)}</dd><dt>Schema version</dt><dd>${escapeHtml(view.schema_version)}</dd><dt>Content hash</dt><dd>${escapeHtml(view.content_hash||'—')}</dd></dl><h3>View specification</h3><pre>${escapeHtml(JSON.stringify(view.spec,null,2))}</pre><h3>Materialization manifest</h3><pre>${escapeHtml(JSON.stringify(view.manifest,null,2))}</pre>`;$('#analysis-view-details-modal').showModal()}catch(error){notice(error.message)}};
$('#close-analysis-view-details').onclick=()=>$('#analysis-view-details-modal').close();
...
function updateGraphCandidateSelection(){const selected=$$('.candidate-check:checked').length;$('#compare-discovery').disabled=selected<2;return selected}
function setGraphCandidateSelection(selected){$$('.candidate-check').forEach(item=>{item.checked=selected});updateGraphCandidateSelection()}
$('#select-all-graph-candidates').onclick=()=>setGraphCandidateSelection(true);
$('#clear-graph-candidates').onclick=()=>setGraphCandidateSelection(false);
...
function renderGraphCandidates(){const target=$('#graph-candidates');if(!target)return;target.innerHTML=state.graphCandidates.length?`<table><thead><tr><th></th><th>Kind / FIXED</th><th>Source / Parent</th><th>Type / Origin</th><th>Status</th><th>Outcome</th><th>Summary</th><th>操作</th></tr></thead><tbody>${state.graphCandidates.map(c=>`<tr><td><input class="candidate-check" type="checkbox" data-kind="${c.candidate_kind}" value="${c.candidate_id}"></td><td>${c.candidate_kind}<br><span class="status">${c.fixed?'FIXED':'—'}</span></td><td>${escapeHtml(c.parent_graph_version_id||c.source_result_id||'root')}</td><td>${c.graph_type}<br>${c.graph_origin}</td><td>${escapeHtml(c.version_status||c.scientific_status||'—')}</td><td>${escapeHtml(c.designated_outcome_node||'未指定')}</td><td>${c.summary.node_count} nodes / ${c.summary.edge_count} edges<br>${escapeHtml(c.warnings.join?.(', ')||'')}</td><td><button type="button" onclick="inspectCandidate('${c.candidate_kind}','${c.candidate_id}')">グラフを確認/編集する</button></td></tr>`).join('')}</tbody></table>`:'Graph Candidateはありません';$$('.candidate-check').forEach(item=>item.onchange=updateGraphCandidateSelection);updateGraphCandidateSelection()}
function setGraphModalFeedback(message=''){$('#graph-modal-feedback').textContent=message}
function mermaidMarkdownSource(graph){if(!graph)return '';const nodes=[...(graph.nodes||[])].map(String).sort((left,right)=>left.localeCompare(right)),ids=new Map(nodes.map((node,index)=>[node,`node_${index}`])),escapeLabel=value=>String(value).replace(/\\/g,'\\\\').replace(/"/g,'\\"'),edges=[...(graph.edges||[])].sort((left,right)=>`${left.source}\u0000${left.target}\u0000${left.endpoint_source}\u0000${left.endpoint_target}`.localeCompare(`${right.source}\u0000${right.target}\u0000${right.endpoint_source}\u0000${right.endpoint_target}`));return ['```mermaid','flowchart LR',...nodes.map(node=>`  ${ids.get(node)}["${escapeLabel(node)}"]`),...edges.map(edge=>`  ${ids.get(String(edge.source))} -->|${edge.endpoint_source}→${edge.endpoint_target}| ${ids.get(String(edge.target))}`),'```'].join('\n')}
window.inspectCandidate=inspectCandidate;
async function inspectCandidate(kind,id,{preserveFeedback=false}={}){if(!preserveFeedback)setGraphModalFeedback();try{const candidate=await api(`/projects/${state.project.project_id}/graph-candidates/${kind}/${id}`);state.graphCandidate=candidate;state.sourceGraph=structuredClone(candidate.graph);state.editingGraph=structuredClone(candidate.graph);$('#graph-meta').innerHTML=`<div class="badge-row"><span class="status">${candidate.graph_type}</span><span class="status">${candidate.graph_origin}</span><span class="status">${candidate.version_status||candidate.scientific_status}</span></div><p>Source: ${escapeHtml(candidate.source_result_id||'—')} / Parent: ${escapeHtml(candidate.parent_graph_version_id||'—')}</p><p>Outcome: ${escapeHtml(candidate.designated_outcome_node||'未指定')}</p>`;$('#graph-mermaid-source').value=mermaidMarkdownSource(candidate.graph);$('#export-graph-mermaid').disabled=!candidate.graph;$('#graph-outcome').value=candidate.designated_outcome_node||'';$('#graph-rationale').value=candidate.summary.edit_rationale||'';$('#adopt-graph').hidden=candidate.candidate_kind!=='DISCOVERY_RESULT';$('#save-graph').disabled=!(candidate.allowed_actions.can_edit||candidate.allowed_actions.can_create_child);$('#fix-graph').disabled=!candidate.allowed_actions.can_fix;$('#add-edge').disabled=!(candidate.allowed_actions.can_edit||candidate.allowed_actions.can_create_child);$('#save-graph').textContent=candidate.allowed_actions.can_edit?'DRAFTを更新':'子DRAFTを作成';renderGraphEditor();$('#graph-modal').showModal()}catch(error){notice(error.message)}}
$('#close-graph-modal').onclick=()=>$('#graph-modal').close();
$('#export-graph-mermaid').onclick=()=>{const candidate=state.graphCandidate,source=mermaidMarkdownSource(candidate?.graph);if(!candidate||!source)return notice('exportするGraph Candidateを選択してください');const blob=new Blob([source],{type:'text/markdown;charset=utf-8'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=`graph-candidate-${candidate.candidate_id}.md`;link.click();URL.revokeObjectURL(link.href);setGraphModalFeedback(`Mermaid Markdownをexportしました: ${candidate.candidate_id}`)};
async function comparisonExecutionMetadata(candidate){const result=state.results.find(item=>item.result_id===candidate.source_result_id);if(!result?.execution?.execution_id)return null;return api(`/executions/${result.execution.execution_id}/prefill`)}
function comparisonAlgorithmSummary(metadata){if(!metadata)return 'Algorithm / parameters: —';const parameters=Object.entries(metadata.parameters||{}).map(([name,value])=>`${name}=${JSON.stringify(value)}`);return `Algorithm: ${metadata.algorithm_or_estimator} / Parameters: ${parameters.join(', ')||'—'}`}
async function compareGraphCandidates(){const refs=$$('.candidate-check:checked').map(item=>({candidate_kind:item.dataset.kind,candidate_id:item.value}));if(refs.length<2)return notice('2件以上選択してください');try{const comparison=await api(`/projects/${state.project.project_id}/graph-candidate-comparisons/query`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({candidate_refs:refs})});$('#comparison-tabs').innerHTML=comparison.candidates.map((candidate,index)=>`<button type="button" data-index="${index}" aria-current="false">${escapeHtml(candidate.summary.name||candidate.candidate_id.slice(0,8))}</button>`).join('');const show=async index=>{const candidate=comparison.candidates[index];$$('#comparison-tabs button').forEach(button=>{const current=Number(button.dataset.index)===index;button.classList.toggle('current-comparison-candidate',current);button.setAttribute('aria-current',current?'true':'false')});renderGraphVisual(candidate.graph,$('#comparison-graph'),candidate.designated_outcome_node);$('#comparison-summary').innerHTML=`<p><b>Current comparison candidate</b>: ${escapeHtml(candidate.candidate_kind)} / ${escapeHtml(candidate.candidate_id)}</p><p><b>${candidate.graph_type}</b> / ${candidate.graph_origin} / ${candidate.summary.node_count} nodes / ${candidate.summary.edge_count} edges / Outcome: ${escapeHtml(candidate.designated_outcome_node||'未指定')}</p><p data-comparison-algorithm>Algorithm / parameters: 読み込み中</p><h3>Compatibility</h3><p>${comparison.compatibility.compatible?'比較可能':`比較不能: ${escapeHtml(comparison.compatibility.reasons.join(', '))}`}</p><h3>Structure diff</h3>${comparison.compatibility.compatible?`<pre>${escapeHtml(JSON.stringify(comparison.differences,null,2))}</pre>`:'個別タブのGraph表示は利用できます。'}`;try{const metadata=await comparisonExecutionMetadata(candidate),target=$('[data-comparison-algorithm]');if(target&&Number($('#comparison-tabs button.current-comparison-candidate')?.dataset.index)===index)target.textContent=comparisonAlgorithmSummary(metadata)}catch(error){const target=$('[data-comparison-algorithm]');if(target&&Number($('#comparison-tabs button.current-comparison-candidate')?.dataset.index)===index)target.textContent=`Algorithm / parameters: ${error.message}`}};$$('#comparison-tabs button').forEach(button=>button.onclick=()=>{void show(Number(button.dataset.index))});void show(0);$('#graph-comparison-modal').showModal()}catch(error){notice(error.message)}}
...
function selectedInferenceDataset(){const id=$('#inference-form').elements.dataset_version_id.value;return state.datasets.find(item=>item.dataset_version_id===id)}
function reconcileInferenceTreatment({announce=false}={}){const field=$('#inference-treatment'),dataset=selectedInferenceDataset(),current=field.value,schema=dataset?.schema||{},columns=Object.keys(schema);if(!columns.length){field.innerHTML='<option value="">Dataset/schemaを選択してください</option>';field.value='';field.disabled=true;return}field.innerHTML='<option value="">Treatmentを選択</option>'+columns.map(name=>`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');field.disabled=false;if(columns.includes(current)){field.value=current;return}field.value='';if(announce&&current)notice('Dataset Version変更によりschemaに存在しないTreatmentを解除しました。')}
