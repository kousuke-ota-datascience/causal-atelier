...
def test_application_does_not_import_infrastructure_package() -> None:
    application = REPOSITORY_ROOT / "src/causal_atelier/application"
    violations = [
        path
        for path in application.rglob("*.py")
        if "causal_atelier.infrastructure" in path.read_text(encoding="utf-8")
    ]
    assert violations == []


def test_api_routers_depend_on_metadata_port_and_domain_entities() -> None:
    routers = REPOSITORY_ROOT / "src/causal_atelier/interfaces/api/routers"
