...
from ariadne.application.pipeline.artifacts import ArtifactRegistry
from ariadne.interfaces.cli.pipeline import parse_args
from ariadne.application.pipeline.planning import ExecutionPlan, PipelinePlanner, StagePlan
from ariadne.application.pipeline.strategies import DryRunStrategy, ValidateOnlyStrategy
from ariadne.application.pipeline.validation import CrossStageValidator
...
def test_cli_full_run_smoke_writes_manifests_and_report(tmp_path: Path) -> None:
    ...
    assert discovery_data["run_label"] == "pytest-full-run"
    assert inference_data["run_label"] == "pytest-full-run"
...
def test_production_code_has_no_old_package_imports() -> None:
    ...
    roots = [PROJECT_ROOT / "src/ariadne", PROJECT_ROOT / "experiments"]
    text = "\n".join(
    ...
    assert not (PROJECT_ROOT / "src/ariadne/inference").exists()
    assert (
        PROJECT_ROOT / "src/ariadne/causal/inference/discovery_artifacts"
    ).exists()
