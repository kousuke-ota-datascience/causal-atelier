...
def test_dry_run_is_idempotent_and_contains_an_immutable_plan(web) -> None:
    ...
    first = client.post(
        "/api/v1/executions", json=request, headers={"Idempotency-Key": "same-request"}
    )
    second = client.post(
        "/api/v1/executions", json=request, headers={"Idempotency-Key": "same-request"}
    )
...
def test_retry_adds_an_attempt_without_overwriting_failure_history(web) -> None:
    ...
    failed = client.get(f"/api/v1/executions/{execution_id}").json()
    assert failed["status"] == "FAILED"

    retried = client.post(f"/api/v1/executions/{execution_id}/retry")
    assert retried.status_code == 202
    assert retried.json()["id"] == execution_id
    assert worker.run_once()
    ...
    with app.state.database.session() as session:
        ...
        stage = session.scalar(select(m.StageExecution).where(m.StageExecution.execution_id == execution_id))
        attempts = session.scalars(
            select(m.StageAttempt)
            .where(m.StageAttempt.stage_execution_id == stage.id)
...
def test_analysis_ready_discovery_graph_and_result_navigation(web) -> None:
    ...
    for _ in range(5):
        worker.run_once()
        if client.get(f"/api/v1/executions/{execution_id}").json()["status"] in {
            "SUCCEEDED",
            "FAILED",
        }:
            break
    completed = client.get(f"/api/v1/executions/{execution_id}").json()
    assert completed["status"] == "SUCCEEDED", completed.get("error_summary")
    run_results = client.get(f"/api/v1/executions/{execution_id}/results").json()
    ...
    inference_execution_id = inference_run.json()["id"]
    for _ in range(3):
        worker.run_once()
        inference_status = client.get(
            f"/api/v1/executions/{inference_execution_id}"
        ).json()["status"]
        if inference_status in {"SUCCEEDED", "FAILED"}:
            break
    assert inference_status == "SUCCEEDED"
    inference_results = client.get(
        f"/api/v1/executions/{inference_execution_id}/results"
    ).json()
    ...
    treatment_execution_id = treatment_run.json()["id"]
    for _ in range(3):
        worker.run_once()
        treatment_status = client.get(
            f"/api/v1/executions/{treatment_execution_id}"
        ).json()["status"]
        if treatment_status in {"SUCCEEDED", "FAILED"}:
            break
    assert treatment_status == "SUCCEEDED"
    treatment_results = client.get(
        f"/api/v1/executions/{treatment_execution_id}/results"
    ).json()
    ...
    with app.state.database.session() as session:
        ...
        requested = session.get(m.StageExecutionInputPreparation, stage_id)
        actual = session.get(
