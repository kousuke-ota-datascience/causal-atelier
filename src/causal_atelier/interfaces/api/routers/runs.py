"""Asynchronous run lifecycle, event, artifact, and result endpoints."""
...
from causal_atelier.application.run_execution.services import (
    ConflictError,
...
from causal_atelier.interfaces.api.schemas import RunCreate

from .common import get_or_404, model_dict


router = APIRouter(tags=["runs", "results", "artifacts"])
...
def create_run(
    ...
    try:
        run, reused = RunService(session).create(
...
def list_runs(
    ...
    query = select(m.Run).where(m.Run.project_id == project_id)
    if run_status:
        query = query.where(m.Run.status == run_status)
    if experiment_id:
        query = query.where(m.Run.experiment_id == experiment_id)
    total = session.scalar(select(func.count()).select_from(query.subquery())) or 0
    items = session.scalars(
        query.order_by(m.Run.submitted_at.desc())
        .offset((page - 1) * limit)
        .limit(limit)
    ).all()
    return {
...
def cancel_run(
    ...
    if run.status != "CANCEL_REQUESTED":
        ...
        RunService(session).add_event(run.id, "CANCEL_REQUESTED", {"best_effort": True})
        session.add(
            m.OutboxEvent(
                aggregate_type="RUN",
                aggregate_id=run.id,
        ...
        add_audit(
            ...
            actor_user_id=user.id,
            action="RUN_CANCEL_REQUEST",
            resource_type="RUN",
            resource_id=run.id,
            request_id=request.state.request_id,
        )
    return {"run_id": run.id, "status": run.status, "best_effort": True}
...
def retry_run(
    ...
    require_project_role(session, user, run.project_id, "ANALYST")
    try:
        retried = RunService(session).retry(run, user.id)
    except ConflictError as exc:
        raise HTTPException(status.HTTP_409_CONFLICT, str(exc)) from exc
...
def get_run_events(
    ...
    events = session.scalars(
        select(m.RunEvent)
        .where(m.RunEvent.run_id == run.id, m.RunEvent.sequence_number > after_sequence)
        .order_by(m.RunEvent.sequence_number)
...
def get_run_artifacts(
    ...
    artifacts = session.scalars(
        ...
        .join(
            ...
            m.StageRunArtifactOutput.artifact_id == m.Artifact.id,
        )
        .join(m.StageRun, m.StageRun.id == m.StageRunArtifactOutput.stage_run_id)
...
def get_discovery_result(
    ...
    stage = get_or_404(session, m.StageRun, result.stage_run_id)
...
def get_edge_weight_result(
    ...
    diagnostics = session.scalars(
        select(m.DiagnosticSummary).where(m.DiagnosticSummary.stage_run_id == stage.id)
    ).all()
    return {
        **model_dict(result),
...
def _run_response(session: Session, run: m.Run) -> dict:
    ...
    validations = session.scalars(
        select(m.ValidationRun).where(m.ValidationRun.run_id == run.id)
    ).all()
    validation_payload = []
    for validation in validations:
        issues = session.scalars(
            select(m.ValidationIssueRecord)
            .where(m.ValidationIssueRecord.validation_run_id == validation.id)
