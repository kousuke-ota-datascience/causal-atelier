...
class RunStateManager:
    ...
    def create_attempt(
        self, session: Session, stage: m.StageRun
    ) -> m.StageAttempt:
        stale_attempts = session.scalars(
            select(m.StageAttempt).where(
                m.StageAttempt.stage_run_id == stage.id,
    ...
    def finish_cancelled(
        ...
        for stage in session.scalars(
            select(m.StageRun).where(m.StageRun.run_id == run.id)
        ).all():
            if stage.status not in {"SUCCEEDED", "FAILED", "CANCELLED"}:
                stage.status = "CANCELLED"
                stage.finished_at = m.utcnow()
        service.add_event(run.id, "RUN_CANCELLED", {"best_effort": True})

    def bind_dependency_artifacts(
        self, session: Session, stage: m.StageRun
    ) -> None:
        dependencies = session.scalars(
            select(m.StageRunDependency).where(
                m.StageRunDependency.stage_run_id == stage.id
            )
        ).all()
        for dependency in dependencies:
            upstream = session.get(m.StageRun, dependency.depends_on_stage_run_id)
            outputs = session.scalars(
                select(m.StageRunArtifactOutput).where(
                    m.StageRunArtifactOutput.stage_run_id == upstream.id
                )
            ).all()
            for output in outputs:
                input_name = f"{upstream.stage_key}.{output.output_name}"[:255]
                if session.get(m.StageRunArtifactInput, (stage.id, input_name)) is None:
                    session.add(
...
__all__ = ["OutboxConsumer", "RunStateManager"]
