...
from causal_atelier.causal.inference.estimators.edge_weight import EdgeWeightEstimator
from causal_atelier.causal.inference.estimators.treatment_effect import (
...
from causal_atelier.preprocessing.discovery.config import (
...
class AnalysisReadyExecutor:
    ...
    def execute(
        ...
        preparation = session.get(m.StageRunInputPreparation, stage.id)
        ...
        for config_input in session.scalars(
            select(m.StageRunConfigInput).where(
                m.StageRunConfigInput.stage_run_id == stage.id
            )
        ).all():
            version = session.get(
    ...
    @staticmethod
    def _graph_version(session: Session, stage: m.StageRun) -> m.CausalGraphVersion:
