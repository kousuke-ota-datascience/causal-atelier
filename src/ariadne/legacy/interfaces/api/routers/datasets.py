...
from ariadne.application.run_execution.services import add_audit
...
@router.get("/etl-runs/{execution_id}/dataset-comparison")
def compare_etl_datasets(
    ...
    stage_ids = session.scalars(
        select(m.StageExecution.id).where(
            m.StageExecution.execution_id == run.id, m.StageExecution.stage_type == "ETL"
        )
    ).all()
    input_ids = session.scalars(
        select(m.StageExecutionDatasetInput.dataset_version_id).where(
            m.StageExecutionDatasetInput.stage_execution_id.in_(stage_ids)
        )
    ).all()
    output_versions = session.scalars(
        select(m.DatasetVersion).where(
            m.DatasetVersion.origin_stage_execution_id.in_(stage_ids)
        )
    ).all()
