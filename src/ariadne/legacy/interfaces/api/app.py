"""FastAPI application factory for the ariadne control plane."""
...
from ariadne.infrastructure.artifact_store import build_artifact_store
...
from ariadne.interfaces.api.dependencies import seed_roles

from .routers import causal_graphs, configurations, datasets, executions, projects, visualizations


logger = logging.getLogger("ariadne.api")
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(
        title="ariadne Control Plane API",
        version=__version__,
        description=(
...
def main() -> None:
    uvicorn.run(
        "ariadne.interfaces.api.app:app", host="0.0.0.0", port=8000, reload=False
    )
