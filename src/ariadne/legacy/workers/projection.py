...
from ariadne.application.ports import ArtifactStore, DataQuery
...
class WorkerProjectionService:
    ...
    def _project_discovery(
        self, session: Session, stage: m.StageExecution, output_dir: Path
    ) -> None:
    ...
    def _project_edge_weights(
        self, session: Session, stage: m.StageExecution, artifacts: dict[str, Path]
    ) -> None:
...
def _first_dataset_id(session: Session, stage_id: str) -> str | None:
    return session.scalar(
        select(m.StageExecutionDatasetInput.dataset_version_id).where(
...
def _artifact_for_output(
    ...
    output = session.get(m.StageExecutionArtifactOutput, (stage_id, name))
    return session.get(m.Artifact, output.artifact_id) if output else None


def _graph_input(session: Session, stage_id: str) -> m.StageExecutionGraphInput | None:
    return session.scalar(
...
def _current_preparation_id(session: Session, stage_id: str) -> str | None:
    attempt = session.scalar(
        ...
        .where(m.StageAttempt.stage_execution_id == stage_id)
