...
class DatasetVersion(Base):
    ...
    origin_stage_execution_id: Mapped[str | None] = mapped_column(
        ForeignKey("stage_execution.id", use_alter=True)
    )
    created_by: Mapped[str] = mapped_column(ForeignKey("app_user.id"))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utcnow
...
class DataProfile(Base):
    ...
    profiler_name: Mapped[str] = mapped_column(String(128), default="ariadne")
...
class Execution(Base):
    ...
    retry_of_execution_id: Mapped[str | None] = mapped_column(ForeignKey("execution.id"))
    ...
    mlflow_tracking_status: Mapped[str] = mapped_column(String(32), default="PENDING")
    mlflow_tracking_error: Mapped[str | None] = mapped_column(Text)
    __table_args__ = (
        Index(
            "uq_execution_project_idempotency",
            "project_id",
            "idempotency_key",
            unique=True,
        ...
        Index(
            "idx_execution_project_status_submitted", "project_id", "status", "submitted_at"
        ),
        Index(
            "uq_execution_mlflow_run_id",
            "mlflow_run_id",
            unique=True,
            postgresql_where=(mlflow_run_id.is_not(None)),
            sqlite_where=(mlflow_run_id.is_not(None)),
        ),
        CheckConstraint(
            "mlflow_tracking_status IN ('NOT_REQUIRED','PENDING','ACTIVE','FINISHED','ERROR')",
            name="ck_execution_mlflow_tracking_status",
        ),
    )
...
class ExecutionPlanRecord(Base):
    ...
    execution_id: Mapped[str] = mapped_column(ForeignKey("execution.id"), primary_key=True)
...
class StageExecution(Base):
    ...
    reused_from_stage_execution_id: Mapped[str | None] = mapped_column(
        ForeignKey("stage_execution.id")
    )
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    finished_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    error_code: Mapped[str | None] = mapped_column(String(128))
    error_summary: Mapped[str | None] = mapped_column(Text)
    __table_args__ = (
        UniqueConstraint("execution_id", "stage_key"),
        UniqueConstraint("execution_id", "ordinal"),
    )


class StageExecutionDependency(Base):
    __tablename__ = "stage_execution_dependency"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
    depends_on_stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
...
class StageAttempt(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    stage_execution_id: Mapped[str] = mapped_column(ForeignKey("stage_execution.id"), index=True)
    ...
    __table_args__ = (UniqueConstraint("stage_execution_id", "attempt_number"),)


class StageExecutionDatasetInput(Base):
    __tablename__ = "stage_execution_dataset_input"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
...
class StageExecutionConfigInput(Base):
    __tablename__ = "stage_execution_config_input"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
    input_name: Mapped[str] = mapped_column(String(255), primary_key=True)
...
class StageExecutionArtifactInput(Base):
    __tablename__ = "stage_execution_artifact_input"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
    input_name: Mapped[str] = mapped_column(String(255), primary_key=True)
...
class StageExecutionInputPreparation(Base):
    __tablename__ = "stage_execution_input_preparation"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
...
class StageExecutionParameter(Base):
    __tablename__ = "stage_execution_parameter"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
...
class StageExecutionArtifactOutput(Base):
    __tablename__ = "stage_execution_artifact_output"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
    output_name: Mapped[str] = mapped_column(String(255), primary_key=True)
...
class ManifestRecord(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    execution_id: Mapped[str] = mapped_column(ForeignKey("execution.id"), index=True)
    stage_execution_id: Mapped[str | None] = mapped_column(ForeignKey("stage_execution.id"))
...
class ValidationExecution(Base):
    __tablename__ = "validation_execution"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    execution_id: Mapped[str] = mapped_column(ForeignKey("execution.id"), index=True)
    stage_execution_id: Mapped[str | None] = mapped_column(ForeignKey("stage_execution.id"))
...
class ValidationIssueRecord(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    validation_execution_id: Mapped[str] = mapped_column(
        ForeignKey("validation_execution.id"), index=True
    )
...
class DiscoveryResult(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    stage_execution_id: Mapped[str] = mapped_column(ForeignKey("stage_execution.id"), unique=True)
    dataset_version_id: Mapped[str] = mapped_column(ForeignKey("dataset_version.id"))
...
class StageExecutionGraphInput(Base):
    __tablename__ = "stage_execution_graph_input"
    stage_execution_id: Mapped[str] = mapped_column(
        ForeignKey("stage_execution.id"), primary_key=True
    )
    input_name: Mapped[str] = mapped_column(String(255), primary_key=True)
