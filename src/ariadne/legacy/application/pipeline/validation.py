...
from ariadne.causal.design import CausalDesign
from ariadne.application.pipeline.configuration import load_yaml_mapping
from ariadne.preprocessing.common import FeatureRole, FeatureSemanticsCatalog
from ariadne.shared.validation import ValidationIssue, ValidationResult, ValidationSeverity
...
class CrossStageValidator:
    ...
    def validate_discovery_manifest_schema(self, manifest_path: Path) -> list[ValidationIssue]:
        ...
        required = {"run_label", "stage", "resolved_output_dir", "artifacts"}
        # Accept execution_id or run_id as backward-compatible aliases from older manifests.
        if "run_label" not in manifest:
            if "execution_id" in manifest:
                manifest = {**manifest, "run_label": manifest["execution_id"]}
            elif "run_id" in manifest:
                manifest = {**manifest, "run_label": manifest["run_id"]}
