...
from ariadne.application.pipeline.artifacts import ArtifactRegistry, ArtifactSpec
from ariadne.application.pipeline.configuration import (
    load_yaml_mapping,
...
class PipelinePlanner:
    ...
    def build_plan(self, args: Any, *, strategy_name: str) -> ExecutionPlan:
        ...
        run_label = (
            ...
            or pipeline.get("run_label")
            or pipeline.get("run_id")
            # No UUID fallback: CLI must not generate pseudo Ariadne execution IDs.
            # When no label is provided, run_label is None.
        ) or None
    ...
    def _build_discovery_stage(
        ...
        *,
        ...
        return StagePlan(
            ...
            metadata={"run_label": run_label, "random_seed": random_seed},
        )
