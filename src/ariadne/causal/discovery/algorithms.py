...
# Sentinel distinguishing "not supplied" from None in run_pc's background_knowledge.
_SENTINEL = object()
...
class CausalDiscovery:
    ...
    Args:
        ...
        feature_config: Feature configuration used to construct tiers.  May be
            ``None`` when background knowledge is supplied externally via
            ``run_all`` or ``run_pc``.
    ...
    def __init__(
        ...
        *,
        ...
        ges_score_func: str = "local_score_BIC",
    ) -> None:
        ...
        Args:
            ...
            random_seed: Random seed for bootstrap sampling.
            pc_discrete_bins: Number of quantile bins for discrete PC tests.
            ges_max_p: Maximum number of parents for GES (None = unlimited).
            ges_score_func: Score function name for GES.
        ...
        self.ges_max_p = ges_max_p
        self.ges_score_func = ges_score_func

    def run_all(
        self,
        frame: pd.DataFrame,
        *,
        background_knowledge: object | None = None,
    ) -> dict[str, DiscoveryResult]:
        """設定された全アルゴリズムを実行する。

        Args:
            frame: Standardized discovery input.
            background_knowledge: Pre-built causal-learn BackgroundKnowledge object.
                When provided, overrides the internal background knowledge built
                from feature_config.  Ignored by algorithms other than PC.
        ...
        for algorithm in self.algorithms:
            ...
            try:
                if algorithm == "pc":
                    causal_graph, edges = self.run_pc(
                        frame, background_knowledge=background_knowledge
                    )
                else:
    ...
    def run_pc(
        ...
        *,
        ...
        background_knowledge: object | None = _SENTINEL,
    ):
        """PC アルゴリズムを実行する。
        ...
        Args:
            ...
            background_knowledge: Pre-built causal-learn BackgroundKnowledge.
                When omitted, the instance builds it from feature_config if
                ``use_background_knowledge`` is True.
        ...
        if background_knowledge is _SENTINEL:
            # Default path: build from feature_config when enabled.
            resolved_bk = (
            ...
            # Caller-supplied pre-built background knowledge (may be None).
            resolved_bk = background_knowledge
        causal_graph = pc(
            pc_frame.to_numpy(),
            alpha=self.alpha if alpha is None else alpha,
            indep_test=self.pc_indep_test,
            stable=True,
            background_knowledge=resolved_bk,
    ...
    def run_ges(self, frame: pd.DataFrame):
        ...
        result = ges(
            frame.to_numpy(),
            score_func=self.ges_score_func,
            maxP=self.ges_max_p,
