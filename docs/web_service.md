...
```bash
export CAUSAL_ATELIER_DATABASE_URL='postgresql+psycopg://user:password@host/database'
uv run alembic upgrade head
```

`CAUSAL_ATELIER_AUTO_CREATE_SCHEMA=true` はSQLiteを用いるローカル開発・test向けです。
...
```bash
export CAUSAL_ATELIER_AUTH_MODE=oidc
export CAUSAL_ATELIER_OIDC_ISSUER=https://identity.example.com/
export CAUSAL_ATELIER_OIDC_AUDIENCE=causal-atelier
export CAUSAL_ATELIER_OIDC_JWKS_URL=https://identity.example.com/.well-known/jwks.json
```
...
小規模objectは同期処理し、`CAUSAL_ATELIER_QUERY_ASYNC_THRESHOLD_BYTES`を超えるobject、
...
```bash
export CAUSAL_ATELIER_ARTIFACT_BACKEND=LOCAL
export CAUSAL_ATELIER_ARTIFACT_ROOT=/srv/causal-atelier/objects
```

S3互換adapter:

```bash
export CAUSAL_ATELIER_ARTIFACT_BACKEND=S3
export CAUSAL_ATELIER_S3_BUCKET=causal-atelier-artifacts
export CAUSAL_ATELIER_S3_ENDPOINT_URL=https://s3.example.com  # AWS S3では省略可
...
```bash
export CAUSAL_ATELIER_ARTIFACT_BACKEND=AZURE_BLOB
export CAUSAL_ATELIER_AZURE_BLOB_CONTAINER=causal-atelier-artifacts
export CAUSAL_ATELIER_AZURE_STORAGE_CONNECTION_STRING='DefaultEndpointsProtocol=...'
```

connection stringの代わりに`CAUSAL_ATELIER_AZURE_STORAGE_ACCOUNT_URL`と
`CAUSAL_ATELIER_AZURE_STORAGE_CREDENTIAL`も指定できます。S3/Azure Blob objectは
...
| variable | default |
|---|---|
| `CAUSAL_ATELIER_DATABASE_URL` | `.causal-atelier/metadata.db`のSQLite URL |
| `CAUSAL_ATELIER_ARTIFACT_ROOT` | `.causal-atelier/objects` |
| `CAUSAL_ATELIER_WORKSPACE_ROOT` | `.causal-atelier/workspaces` |
| `CAUSAL_ATELIER_ARTIFACT_BACKEND` | `LOCAL` |
| `CAUSAL_ATELIER_AZURE_BLOB_CONTAINER` | Azure Blob利用時は必須 |
| `CAUSAL_ATELIER_AZURE_STORAGE_CONNECTION_STRING` | Azure Blob接続情報 |
| `CAUSAL_ATELIER_AUTH_MODE` | `development` |
| `CAUSAL_ATELIER_QUERY_ASYNC_THRESHOLD_BYTES` | `100000000` |
| `CAUSAL_ATELIER_QUERY_MAX_RESULT_ROWS` | `10000` |
| `CAUSAL_ATELIER_QUERY_MAX_SAMPLE_ROWS` | `50000` |
| `CAUSAL_ATELIER_WORKER_LEASE_SECONDS` | `300` |
