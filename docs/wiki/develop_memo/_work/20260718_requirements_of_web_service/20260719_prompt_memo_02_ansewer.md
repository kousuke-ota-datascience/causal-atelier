...
結論として、現状の`frontend/`をリポジトリ直下に置く構成は妥当である。`src/ariadne`配下へ移す必要はない。
...
`src/ariadne`配下が適切になるのは、FrontendをFastAPIが同一process・同一Python packageから配信する場合である。その場合は、例えば次の配置が考えられる。

```text
src/ariadne/interfaces/web/static/
```
