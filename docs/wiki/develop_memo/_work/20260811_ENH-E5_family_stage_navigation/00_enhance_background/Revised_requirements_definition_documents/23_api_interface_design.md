...
- API base: `/api/v1`
- Planning baseline branch: `feature/ariadne_mvp_e5`
- Planning baseline SHA: `46122c68333df03680b97c253a7b5d32bf9393e7`

本書は、ENH-E5で変更しないinterfaceも含め、ENH-E5適用後に有効であるべきAPI / interface contractを本文内で説明する。`既存APIを使う`、`既存contractを維持する`という記述だけでinterfaceの内容を外部文書やsource codeへ委譲しない。
...
14. current implementation factとENH-E5 target contractを区別する。target候補が未承認の場合は、本書内に選択肢・責務境界・freeze条件を明示する。

## 2. Authentication / Authorization

### 2.1 Current request identity

Planning baselineのFastAPI applicationには共通Bearer/OIDC authentication middlewareは存在しない。
...
ENH-E5はAuthentication方式を変更するEnhancementではない。したがって`Authorization: Bearer`やproduction OIDCが既に成立しているものとして設計しない。
...
- ENH-E5ではNavigation改修を理由に、未実装の全API統一認可を既存contractとして記述しない。
- Navigation metadataをProject-scoped APIとして追加する場合は、少なくとも既存Project境界を弱めてはならない。


### 2.3 ENH-E5 Project authorization coverage
...
Planning baselineで明示されている主なmapping:
...
`401 / 413 / 429`はPlanning baselineのcommon error handlerで一般contractとして定義されていないため、既存contractとして列挙しない。
...
したがってENH-E5で共通`?limit=&cursor=&sort=` contractが既に存在するものとして扱わない。新しいNavigation metadata APIがpaginationを必要とする場合は、そのAPI固有contractとして明示する。
...
Project / Research ContextはFamily切替とは独立したProject-global resourceである。Family tab操作でresource identityを変更しない。
...
`research-contexts/{context_id}/usage`のPlanning baseline実装は`AnalysisSpecificationOrm`に加え、historical/compatibility read modelである`FamilyExecutionOrm / FamilyResultOrm`を参照する。canonical `Execution / Result`全体の完全なusage indexではないため、そのように一般化しない。
...
Planning baselineに`/dataset-versions/{id}/profile`専用endpointはない。profile summaryはDataset metadata response内の`profile_summary`として取得する。
...
ENH-E5では`POST/PATCH/validate/fix`の全boundaryで同じtyped filter validatorを利用する。Mismatch responseのcanonical error codeは`FILTER_TYPE_MISMATCH`。
...
| GET | `/projects/{project_id}/analysis-specifications` | Project内Specification一覧。Planning baselineではFamily / Context / Dataset query filterは受けない |
...
Planning baselineで存在するExploratory / Analysis View interfaceは次である。
...
Planning baselineのExploratory plannerは各operationを1 runtime StageからなるExecution Planへ写像する。Navigation `Data Quality` / `Findings`等のために同名runtime endpointを追加せず、Dataset metadata、Result、Artifact、Annotation等のreadを組み合わせてよい。

## 8. Causal Interface

### 8.1 Graph

Causal analysisはGraph Version / candidate graphを扱う。Planning baselineのpublic interfaceは次である。
...
Planning baselineの`CausalPlanner`はcanonical `Execution.operation`から**1 runtime Stageだけを持つcompatibility plan**を生成する。
...
Predictive analysisは`predictive-analysis-spec/1`を入力契約とする。Planning baselineのfull runtime planは`split -> prepare -> train -> evaluate -> optional explain`である。
...
Planning baselineで存在するPredictive support API:
...
### 9.1 ENH-E5 subgroup evaluation response contract
...
現行branchには、canonical/common Execution APIとPredictive workflow向けProject-scoped APIが併存する。ENH-E5はNavigation再構成を理由にこれらを暗黙統合・削除しない。
...
ENH-E5 comparability responseには最低限次を追加する。
...
ENH-E5 canonical lineage read responseは`ResearchContextVersion -> AnalysisSpecification -> ExecutionPlan -> Execution -> StageExecution -> Result -> Artifact`を再構成し、DatasetVersion / AnalysisView / GraphVersion / input Result / base Executionを含める。deterministic structural relationをmanual/generic LineageEdgeとして二重writeしない。
...
ENH-E5ではunscoped legacy routeもArtifactからProjectをresolveして同じmembership authorizationを適用する。Explicit sensitive Artifact/output detailはOWNER/EDITORのみ。
...
```json
{
  "operations": {
    "RUN": {
      "allowed": false,
      "reason_code": "SPEC_NOT_FIXED",
      "message": "Fix the specification first."
    },
    "EDIT": {"allowed": true},
    "EXPORT": {"allowed": true}
  }
...
Stageがnavigation上表示されることと、そのStage内のactionが実行可能であることは別contractとする。
...
Legacy `/explore`, `/predictive`, `/causal`は各Family default Stageへ一方向normalizeする。Explicit Family/Stageとresource semantic mismatchはsilent correctionせずroute errorとする。
...
Planning baselineに独立したpublic`ExecutionEventPublisher / StageEventPublisher`はない。runtime lifecycle authorityはpersistent `Execution / StageExecution` stateである。

Family tab/Navigation route changeをruntime lifecycle eventへ変換しない。

## 15. CLI / Library Interface

Planning baselineのproduct script entry pointは次である。
...
したがってENH-E5では、存在しない次のようなgeneric Resource CLIをcurrent interfaceとして捏造しない。
...
将来generic product CLIを追加する場合は別scopeでinterface contractを設計する。本Enhancementの必須事項は、既存headless scientific CLIおよびPython/backend use caseへ`current_family` / `navigation_stage` / browser routeをrequired inputとして追加しないことである。
...
ENH-E5でidempotency対象とするCommand:
...
- ArtifactStoreとmetadata DBのcross-store compensationはNFR-007=D3/FUTURE。
...
11. persisted `EXECUTE` role / system Operator roleをENH-E5 project authorizationへ追加するinterface
...
- D3 interface（general AuditLog/retention/object-store等）をE5 acceptanceへ混ぜない。
