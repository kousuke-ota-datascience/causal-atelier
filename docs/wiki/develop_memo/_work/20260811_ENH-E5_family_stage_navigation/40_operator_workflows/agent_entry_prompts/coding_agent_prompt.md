...
10_enhance_instruction/
  {{GATE_ID}}/
    06_Ariadne_ENH-E5_{{GATE_ID}}_implementation_instruction.md
```
...
```bash
git commit -m "ENH-E5 Gate {{GATE_ID}} Trial {{TRIAL_NO}} implementation candidate"
git rev-parse HEAD
```
...
```bash
git add docs/wiki/develop_memo/_work/20260811_ENH-E5_family_stage_navigation/20_implementation_reports/{{GATE_ID}}/Trial{{TRIAL_NO}}/E5-{{GATE_ID}}_{{TRIAL_NO}}__implementation_completion.md
git status
git diff --cached
```
...
```bash
git commit -m "ENH-E5 Gate {{GATE_ID}} Trial {{TRIAL_NO}} implementation evidence"
