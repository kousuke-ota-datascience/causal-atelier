...
10_enhance_instruction/
  {{GATE_ID}}/
    07_Ariadne_ENH-E5_{{GATE_ID}}_test_instruction.md
```
...
```text
E5-{{GATE_ID}}_{{TRIAL_NO}}__<TEST_ITEM_ID>.md
```

例:

```text
E5-G00_01__001_candidate_identity.md
E5-G00_01__999_gate_decision.md
```
...
FAIL の場合、failure を implementation defect として evidence 化する。

Test Agent 自身で修正してはならない。

### BLOCKED

candidate identity、contract ambiguity、environment / prerequisite 等により Acceptance を判定できない場合。
...
```bash
git add docs/wiki/develop_memo/_work/20260811_ENH-E5_family_stage_navigation/30_test_report/{{GATE_ID}}/Trial{{TRIAL_NO}}/
git status
...
```bash
git commit -m "ENH-E5 Gate {{GATE_ID}} Trial {{TRIAL_NO}} independent verification evidence"
