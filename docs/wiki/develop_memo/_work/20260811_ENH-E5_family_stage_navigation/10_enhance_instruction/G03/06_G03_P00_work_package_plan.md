...
|---|---|---|---|---|---|
| P01 | 既存form/result operationを保持し、現行Causal UIを7つのsemantic Stage panelへrefactorする。 | NONE | upstream Package完了またはNONE | focused implementation + verification + checkpoint report | Package固有focused test |
| P02 | IdentificationとEstimationのsurface/routeを明示的に分離し、action prerequisiteを保持する。 | P01 | upstream Package完了またはNONE | focused implementation + verification + checkpoint report | Package固有focused test |
| P03 | discovery/effects/diagnostics/sensitivityを含むCausal focused regressionとcandidate assembly input整備。 | P01,P02 | upstream Package完了またはNONE | focused implementation + verification + checkpoint report | Package固有focused test |
