...
| GitHub repository | `git@github.com:kousuke-ota-datascience/ariadne.git` |
| Default branch | `main` |
...
```bash
git commit -m "Initial commit: restructure ariadne"
```
...
```bash
git remote add ariadne git@github.com:kousuke-ota-datascience/ariadne.git
git remote -v
```

すでに `ariadne` が存在する場合は、追加ではなくURLを更新する。

```bash
git remote set-url ariadne git@github.com:kousuke-ota-datascience/ariadne.git
git remote -v
```
...
- ローカルの `main` が `ariadne/main` を追跡している
