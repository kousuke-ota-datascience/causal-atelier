...
```bash
git commit -m "ENH-E6 Gate {{GATE_ID}} Trial {{TRIAL_NO}} {{PACKAGE_ID}} implementation checkpoint"
git rev-parse HEAD
```
...
20_implementation_reports/
  {{GATE_ID}}/
    Trial{{TRIAL_NO}}/
      packages/
        E6-{{GATE_ID}}_{{TRIAL_NO}}_{{PACKAGE_ID}}__status.md
```
...
```bash
git add docs/wiki/develop_memo/_work/20260813_ENH-E6_family_stage_navigation_bugfix/20_implementation_reports/{{GATE_ID}}/Trial{{TRIAL_NO}}/packages/E6-{{GATE_ID}}_{{TRIAL_NO}}_{{PACKAGE_ID}}__status.md
git status
git diff --cached
```
...
```bash
git commit -m "ENH-E6 Gate {{GATE_ID}} Trial {{TRIAL_NO}} {{PACKAGE_ID}} implementation evidence"
