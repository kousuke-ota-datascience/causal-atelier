...
10_enhance_instruction/
  {{GATE_ID}}/
    06_Ariadne_ENH-E6_{{GATE_ID}}_implementation_instruction.md
```
...
```bash
git commit -m "ENH-E6 Gate {{GATE_ID}} Trial {{TRIAL_NO}} implementation candidate"
git rev-parse HEAD
```
...
```bash
git add docs/wiki/develop_memo/_work/20260813_ENH-E6_family_stage_navigation_bugfix/20_implementation_reports/{{GATE_ID}}/Trial{{TRIAL_NO}}/E6-{{GATE_ID}}_{{TRIAL_NO}}__implementation_completion.md
git status
git diff --cached
```
...
```bash
git commit -m "ENH-E6 Gate {{GATE_ID}} Trial {{TRIAL_NO}} implementation evidence"
