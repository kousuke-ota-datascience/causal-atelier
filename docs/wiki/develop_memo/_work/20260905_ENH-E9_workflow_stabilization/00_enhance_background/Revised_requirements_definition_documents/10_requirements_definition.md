...
各Requirementは、Requirementそのものの有効性、現在の実装充足状態、およびdelivery origin/scopeを独立した属性として管理する。
...
| `DEFERRED` | Requirementとしての必要性は維持するが、current mandatory delivery/acceptanceから除外され、将来の実装対象として延期されている。 |
| `RETIRED` | Requirementとして廃止され、current product contractとして維持しない。 |
...
| Value | Meaning |
| --- | --- |
| `IMPLEMENTED` | Requirementを満たす実装が存在し、当該snapshotでcurrent implementationとして確認されている。 |
| `PARTIAL` | Requirementの一部は実装されているが、Requirement全体を満たしていない。 |
| `NOT_IMPLEMENTED` | Requirementを満たす実装が確認されていない。 |
| `UNVERIFIED` | 実装有無またはRequirement充足状態を利用可能なevidenceから確定できていない。 |

##### Delivery

Requirementの実装・主要改訂がどのdelivery scopeに属するかを表す。
...
AriadneはProject / Research Topicを境界として、Research Context、Dataset Version、Analysis View、Exploratory / Predictive / Causalの三つのAnalysis Family、Execution、Result、Artifact、AnnotationおよびLineageを管理する。

Application surfaceは`Project Management`と`Analysis Workspace`へ責務分離する。Project ManagementはProject resource / versioned analysis inputのlifecycleを管理し、Analysis WorkspaceはCurrent Project / Active Research Context / Dataset Version / Analysis ViewからなるAnalysis Contextの下でanalysis execution / presentationを行う。

Analysis WorkspaceではAnalysis Familyをanalytical context、Navigation StageをFamily-local work/view contextとして扱う。Navigation Stageはruntime Execution Stageではない。
...
| --- | --- | --- |
| Project Management | Project lifecycleとProject-local resource管理のsurface責務を提供する | Project、ResearchContextVersion、DatasetVersion、AnalysisView、Result、Artifact、LineageEdge |
...
| Application Navigation | Project ManagementとAnalysis Workspaceを別navigation scopeとしてroute-backedに提示する | browser route / application state（non-persistent） |
| Analytical Navigation | Analysis Workspace内でAnalysis FamilyとFamily-local Navigation Stageを独立したnavigation dimensionとして提示する | AnalysisFamily、Navigation metadata/state（non-persistent） |
...
`/projects/{project_id}`は`/projects/{project_id}/overview`へhistory-replace semanticsでnormalizeする。
...
Supported resource typeは`analysis-specification / execution / result / graph-version`とする。Supported legacy analytical entryは各Family default Stageへ一方向normalizeする。
...
| E2E-11 | Project resourceを管理する | Project List / RegisterからProjectを選択し、Overview / Research Context / Data / Resultsへroute-backedに遷移できる |
| E2E-12 | Analysis Workspaceでcontextを保持して分析する | Current Projectをread-onlyで確認し、Research Context / Dataset Version / Analysis Viewを選択してFamily / Stageを操作でき、context変更だけでFamily / Stage routeが失われない |
...
| FR-104 | UI / Navigation | ApplicationはProject ManagementとAnalysis Workspaceを異なるnavigation scopeとして提供し、Project resource管理とFamily / Stage navigationを同一hierarchyへ混在させない | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-105 | UI / Navigation | Project Management routeとAnalysis routeはいずれも独立URL、direct link、reload、browser Back / Forwardでdeterministicに復元できる | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-106 | UI | Analysis WorkspaceはAnalysis ContextとしてCurrent Project、Active Research Context、Dataset Version、Analysis Viewを明示する | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
...
| FR-111 | UI | current surfaceでkeyboard、deterministic focus、accessible name、error association、non-color semanticsおよびcontrast要件を満たす | MUST | ACTIVE | PARTIAL | ENH-E5 |
...
| FR-139 | Navigation | Overview / Research Context / Data / ResultsをProject Management local navigationとして扱い、Exploratory / Predictive / CausalのAnalytical Familyと同一taxonomyへ混在させない | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
...
| FR-165 | Project Navigation | `/projects/{project_id}/overview`をselected Projectのdefault canonical surfaceとし、`/projects/{project_id}`はhistory-replaceでoverviewへnormalizeする | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-166 | Project Management | selected Projectのlocal navigationはOverview / Research Context / Data / Resultsを提供する | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-167 | Project Management | OverviewはProject metadata / identity / status / archiveを所有し、Project Archiveをselected Project lifecycle operationとして提供する | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-168 | Project Management | DataはDataset / Dataset Version / Schema / Preview / Analysis View lifecycleを所有し、Analysis ViewをFamily横断analysis inputとして管理する | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-169 | Result / Lineage | Results / Lineageはpersisted cross-analysis aggregation、comparison、Artifact、Lineage、Annotationを所有し、Analysis Workspaceはexecution-local / stage-local presentationを担う | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-170 | Analysis Workspace | Analysis WorkspaceをProject Managementとは別surfaceとし、Family / Stage navigationはAnalysis Workspace内だけに表示する | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-171 | Analysis Context | Current Projectはcanonical Analysis URLの`project_id`をauthorityとしAnalysis Workspace内ではread-onlyとする。Project変更はProject Management経由で行う | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-172 | Analysis Context | Research Context / Dataset Version / Analysis ViewはCurrent Projectと整合するexisting resourceから選択し、復元不能時は架空default resourceを生成せずunselectedとする | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-173 | Analysis Context | Dataset Version変更時にselected Analysis Viewが互換でなければAnalysis View selectionを解除し、context変更または不足だけを理由にFamily / Stage routeを書き換えない | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
| FR-174 | Analysis Surface | existing Causal / Exploratory / Predictive capabilityを定義済みStage Contentsから操作可能にし、UI再配置だけを理由にanalysis execution semanticsを変更しない | MUST | ACTIVE | IMPLEMENTED | ENH-E7 |
...
| NFR-012 | Accessibility | current surfaceの主要操作をkeyboardで実行でき、focus/name/error/non-color/contrast semanticsを満たす | — | ACTIVE | PARTIAL | ENH-E5 |
...
| NFR-019 | Documentation | `00/10/21/22/23/30`だけでcurrent product concept / requirement / logical model / design / interface contractを自己完結して理解できる | — | ACTIVE | IMPLEMENTED | ENH-E5 |
...
指定subgroup columnはuntouched TEST populationを各column独立にsliceする。automatic intersection/discovery/fairness frameworkはcurrent mandatory scope外。各recordは`sample_count`を必須とし、uncertaintyはnonparametric percentile bootstrap（confidence=0.95, resamples=1000, deterministic seed）とする。`n < 2`またはvalid resamples < 200ではCIを返さずwarningを返す。metricを計算できない場合はvalue/uncertaintyを`null`とし、値を捏造しない。
...
独立`EXECUTE` roleは追加しない。prediction row / local explanationはpotentially sensitive outputとして扱う。configurable sensitive-column governanceは`DEFERRED`でありcurrent mandatory acceptanceへ含めない。
...
Current contractでは`StageAttempt.effective_random_seed: int | null`を追加し、stochastic Stageのactual seedをattempt単位で保存する。同一logical Stageのretryは同じseedを再利用し、deterministic Stageは`null`とする。
...
### 8.6. Navigation / Analysis Context State

Application navigation stateはProject route scopeとAnalysis route scopeを分離する。

Project Management route state:
...
```text
Current Project       = URL project_id / read-only
Research Context      = Project-scoped selection
Dataset Version       = Project-scoped selection
Analysis View         = Dataset-Version-compatible selection
```

- Current FamilyおよびCurrent Navigation StageはURL / application navigation stateをauthorityとする。
- Current Projectはcanonical Analysis routeの`project_id`をauthorityとする。
- Research Context / Dataset Version / Analysis ViewはCurrent Projectと整合する値だけを復元する。
- Dataset Version変更でAnalysis Viewが互換でなくなった場合はAnalysis View selectionを解除する。
- contextがunselectedであっても、それだけを理由にFamily / Stage routeを書き換えない。
- 必要inputが不足するoperationはexecution unavailableとして表現する。
...
* system-level Operator authorizationはcurrent Project authorization contractには含めず、`DEFERRED / FUTURE`として扱う。

## 10. データ保持要件とDeferred Audit / Retention Boundary

Current contractで有効なcurrent data-retention contractは次のとおりとする。
...
以下はRequirementとして維持するが、current delivery対象には含めない。

* general operational audit trail (`FR-122`): `DEFERRED / FUTURE`
* configurable retention/deletion policy (`FR-126`): `DEFERRED / FUTURE`

したがってCurrent contractでは、general Audit recordの共通field contract、configurable retention期間、Artifactの一般的な物理削除policyおよび削除audit contractを必須acceptanceとして要求しない。
...
1. E2EシナリオがWeb API、Worker、Persistence、FrontendおよびCLI/library境界を通して成立する。
2. ACTIVE Requirementについて、Requirement Levelおよび設計上のverification contractに応じたtest、benchmarkまたはreview evidenceを用意できる。
3. DEFERRED Requirementはcurrent mandatory acceptanceへ混在させない。
4. ACTIVEなScientific Integrity Requirementについて、必要な自動test、scientific benchmarkまたはreview checklistによるverification evidenceを用意できる。
5. `causal-analysis-spec/2`および`predictive-analysis-spec/1`等のcurrent scientific contractを保持する。
6. `AnalysisSpecification.analysis_family`を再利用し、duplicate Family discriminatorを追加しない。
7. AnalysisSpecification / ExecutionPlan / Execution / StageExecutionへNavigation Stage fieldを追加しない。
8. CLI / Python library / backend use case / runtime executorがCurrent Navigation Stageなしでanalysisを実行できる。
9. Product Domain / runtime execution layerからbrowser route / Navigation Stageへの逆依存がない。
10. Project Management / Analysis Workspace / Family / Stageのresponsibilityが同一navigation hierarchyへ再混在していない。
11. Project routeとAnalysis routeのdirect link / reload / Back / Forwardがdeterministicに成立する。
