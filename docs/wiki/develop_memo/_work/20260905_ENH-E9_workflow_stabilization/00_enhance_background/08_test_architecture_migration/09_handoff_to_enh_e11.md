# ENH-E11 向け Test Architecture Handoff

**状態:** `ACTIVE_HANDOFF`  
**対象:** ENH-E11 planning / implementation / verification / operator workflow authoring  
**前提:** repository-wide Comprehensive Test Code MigrationはENH-E12以降へdefer

## 1. Handoffの目的と設計起点

ENH-E11は、repository全体のtest code migration完了を待たずに開始・完了してよい。ただしtest estateは過渡期であり、新architectureと旧配置が共存する。
...
ENH-E11ではこの問題をtest architecture / verification workflow側の設計責務として扱う。Browser E2Eの責務をcritical cross-layer connectivityへ限定し、詳細なcorrectnessをlower deterministic layerへ寄せ、test-side failureをproduct failureから明確に分離する。

## 2. ENH-E11で採用するmigration policy

ENH-E11でrepository全体のtest estateを先に再編しない。

minimum rule:

- ENH-E11で新規作成またはmaterially rebuiltするtestは新architectureを使用する;
- ENH-E11で実際に必要な既存testだけをbatch単位で移行してよい;
- current invariantか未判定の既存testは旧pathに残してよい;
- 「ENH-E11で使わない」だけを理由にarchiveしない;
...
ENH-E11で新しいbehaviorを追加・変更する際、既存behaviorのregression protectionを同時に確認するtestでも、ENH-E11由来のstaging testであれば原則として `tests/enhancement/enh_e11/...` から開始してよい。
...
## 4. Target structureとENH-E11で許容するtransition state
...
### 4.2. ENH-E11で新規・変更testを置く場所

```text
tests/enhancement/enh_e11/<gate>/<layer>/
```

例:

```text
tests/enhancement/enh_e11/g01/unit/
tests/enhancement/enh_e11/g01/contract/
tests/enhancement/enh_e11/g02/frontend/
tests/enhancement/enh_e11/g02/browser_e2e/
```

### 4.3. ENH-E11期間中に許容する共存
...
Gate-level 06はGate claim、required behavior、protected semantics / regression protection、dependency、execution mode、required package setをauthorityとして定義する。Testのphysical pathをGate semanticsにしない。
...
historical path固定を原則避ける。

```text
tests/product/some_old_test.py を実行せよ
```

のような記述をnormative verification authorityにしない。
...
Coding Agent promptには、ENH-E11で新規またはmaterially rebuiltするtestを原則として次へ置くruleを持たせる。
...
```text
tests/enhancement/enh_e11/<gate>/
```

transition stateとして必要なら次も探索する。
...
ENH-E9で作成した `31_blocked_test_repair_01_test_infrastructure_agent_prompt.md` はG02 Trial01専用special handlingであり、ENH-E11へ標準promptとしてコピーしてはならない。

同種の事象がENH-E11で実際に発生した場合、その具体的blockerに対するEnhancement-specific special handlingか、別途generic routeを設計する。
...
## 9. ENH-E11終了時のtest disposition

ENH-E11のtestをすべて即座に `tests/regression/` へ移す必要はない。
...
## 10. ENH-E11でblocking prerequisiteにしないこと

Feature scope上必要でない限り、次をENH-E11開始/完了のblocking prerequisiteにしない。
...
[ ] repository-wide comprehensive migrationをENH-E11のblocking prerequisiteにしていない
```

## 13. Authority boundary

本handoffはAriadne repository固有のtest architecture transition ruleであり、ENH-E11のcanonical requirement、Gate 06/Pxx/07、Gate Decisionをoverrideしない。
