...
本書は Human/operator が、workflow 全体について次を迷わず判断できるようにするための **end-to-end runbook / 実行時アンチョコ** である。
...
Root `README.md` にも一部同じ概念が記載されている。本セクションは仕様正本を重複させるためではなく、**実行中に最低限参照するためのアンチョコ**として残す。
...
### 4.2. `PACKAGE_BLOCKED_CONTRACT_AMBIGUITY`
...
## 9. Browser E2E / environment blocker
...
### 15.2. `README_Appendix_HowToUse_v0.03.md`
...
Root READMEと重複する最小概念は、実行中に参照するアンチョコとしてINTRODUCTIONへ意図的に残す。詳細仕様のauthorityは各contract / artifact側に置く。
