...
ENH-E7 v0.07 は G01 / G02 とも Work Package mode であり、`10_normal_execution_01_single_execution_coding_agent_prompt.md` は `DISABLED_FOR_NORMAL_ENH-E7_EXECUTION` である。そのため ENH-E7 の通常実行では Phase B2 を使用しない。
...
ENH-E7 v0.07では通常実行に対してdisabledである。
...
| A5 | Work Packageあり: `10_enhance_instruction/<GATE>/06_<GATE>_Pxx_*.md` / Work Packageなし: instance-specific single execution contract | Work Package mode: Pxx metadata・Entry criteria・Required implementation・Focused verification・handoff・completion / stop rule。Single execution mode: Gate-wide scope・completion・verification handoff | 選択した実行モードだけで必要十分な実装判断ができる | 不足contractを補完する。Coding Agentにworkflow全体を探索させない |
...
具体prompt / artifact contractはworkflow instance側をauthorityとする。

ENH-E7 v0.07ではsingle executionはdisabledなので、この分岐は使用しない。
...
ENH-E7 v0.07では次のpromptは通常実行に対してdisabledであるため、**G01 / G02でPhase B2を使用してはならない。**

```text
40_operator_workflows/agent_entry_prompts/
10_normal_execution_01_single_execution_coding_agent_prompt.md

Status: DISABLED_FOR_NORMAL_ENH-E7_EXECUTION
```
...
| Gate 06 / 07 | `10_enhance_instruction/<GATE>/06...`, `07...` | 両contractがexecution / verification authorityとして利用可能 |
...
**ENH-E7 v0.07ではこの実行例を使用しない。** 同instanceのsingle execution promptがdisabledだからである。
...
|---|---|---|
| Phase B1の場合: `20_implementation_reports/<GATE>/Trial<TRIAL>/..._implementation_completion_report.md` | `Candidate state`、Fixed Trial Candidate identity | `READY_FOR_TEST` とcandidate identityが確定済み |
| Phase B1の場合: `30_test_report/<GATE>/Trial<TRIAL>/` | Gate 07でrequiredなTest Item report | required Test Itemのevidenceが揃っている |
...
| `30_test_report/<GATE>/Trial<TRIAL>/<ENH>_<GATE>_Trial<TRIAL>_999_gate_decision.md` | `Gate decision` | `PASS` / `FAIL` / `BLOCKED` のいずれかが明示されている |
...
## 8. Phase F — Current State promotion と次 Gate: PASS成果の昇格と次工程への移行
...
ENH-E7 G01 → G02 では、実際に次の2つのLLM作業を実施してからG02 implementationへ進んだ。次版Templateでは、この「次Gate freeze前のsource confirmation」と「discovery結果のcontract反映」をHowToUse上の正式な遷移として扱う。

### 8.2. G02 freeze前の source confirmation をLLMに実施させる

**目的:** G02 Gate contractをfreezeする前に、Planning時点でdeferされたsource factをcurrent source / tests / config / runtime factsから確定する。

**作業後にあるべき状態:** G02 implementation / verification contractを確定するために必要なsource factsとdesign recommendationが分離され、freezeをBLOCKする未解決事項の有無が明示されている。

#### 8.2.1. ENH-E7で実施したプロンプト
...
### 8.3. discovery結果をG02 contractへ反映する

**目的:** source confirmationで確定したFactsをGate contract / Design / Pxxへ反映し、Coding Agentがassigned contractだけで再設計せず実装できる状態にする。

**作業後にあるべき状態:** unresolved semantic blockerが0件で、G02 06 / 07がfrozen authorityとして利用でき、G02 implementation executionへ進める。

#### 8.3.1. ENH-E7で実施したプロンプト
...
この2つは ENH-E7 G02 固有の実例である。別Enhancement / 別Gateでは、deferred questionと対象artifactをそのinstanceに合わせて置換する。
...
[5] Phase E: 999 Gate Decision
     ├─ PASS
     │    ↓
     │  [6] Phase F: Current State promotion
     │    ↓
     │  Next Gate source confirmation / contract freeze
     │    ↓
     │  Next Gate Phase A
     │
     ├─ FAIL
...
- [ ] ENH-E7 v0.07ではこの経路を使用していないことを認識
...
### 15.2. `README_Appendix_HowToUse_v0.06.md`
