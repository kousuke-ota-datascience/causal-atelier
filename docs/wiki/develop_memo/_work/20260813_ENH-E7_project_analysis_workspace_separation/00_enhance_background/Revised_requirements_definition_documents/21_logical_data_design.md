...
| Current Family | navigation state | DB永続化しない | Analysis Workspaceの現在のanalytical Family |
...
### 4.3 Analysis Context logical projection

Analysis Contextは新しいpersistent aggregateではなく、Current Projectとexisting Project-scoped selection/resourceを組み合わせたapplication-level logical projectionである。
...
│  └─ Current Project scoped selection/resource
├─ Dataset Version
│  └─ Current Project scoped selection/resource
└─ Analysis View
   └─ selected Dataset Version compatible selection/resource
```

Logical authority:

| Element | Authority | Persistence rule |
| --- | --- | --- |
| Current Project | canonical Analysis route `project_id` | Analysis Context専用field/resourceを追加しない |
| Active Research Context | existing Project resource / workspace selection | existing lifecycleを利用 |
| Dataset Version | existing Project resource / workspace selection | existing lifecycleを利用 |
| Analysis View | existing Dataset-Version-scoped resource / workspace selection | existing lifecycleを利用 |
| Current Family / Stage | browser URL / navigation state | DBへ永続化しない |

Selection invariants:

1. Research Context / Dataset Version / Analysis ViewはCurrent Projectと整合しなければならない。
2. Analysis Viewはselected Dataset Versionと互換でなければならない。
3. Dataset Version変更時にselected Analysis Viewが互換でない場合、Analysis View selectionを解除する。
4. 有効なselectionを復元できない場合、架空default resourceを生成せずunselectedとする。
5. context selection不足だけを理由にFamily / Navigation Stage routeを書き換えない。
6. Analysis Contextの導入だけを理由にpersistent schema/cardinalityを変更しない。
...
relation targetはFIX時に同一Project内の`ResearchContextVersion`として解決し、自己参照を禁止する。現行実装ではrelation graph全体に対する一般cycle検出は実装されていないため、「特定relationではcycleを禁止する」という追加制約をcurrent contractとして記載しない。
...
#### 5.4.3 Typed filter compatibility
...
- new expression language、derived expressionのfull static typing、Family-specific typingはcurrent mandatory scope外。
...
`runtime_version_json`は最低限次の再構築metadataを保持する。
...
現行実装 DB constraintにおけるoperation別input invariant:
...
`effective_random_seed`はcurrent persistent contractである。stochastic Stageではactual seedをattempt単位で保存し、同一logical Stageのtechnical retryでは同じeffective seedを再利用する。
...
Exploratory Result（`DATA_PROFILE_RESULT / DISTRIBUTION_RESULT / ASSOCIATION_RESULT / GROUP_SUMMARY_RESULT / CHART_RESULT`）は`GENERATED | GENERATED_WITH_WARNINGS`を許可する。`INSUFFICIENT_DATA`は現行実装のpersistent `ScientificStatus`には存在しないため、必要な不足情報はwarning/payload等で表現する。
...
#### 5.13.1 Canonical lineage completion
...
- StageAttempt: unique `(stage_execution_id, attempt_number)`。 `effective_random_seed: int | null`を持つが、unique constraintは追加しない。
...
### 20.7. Analysis Context Logical Projection

Current Project / Research Context / Dataset Version / Analysis Viewのcurrent selection relationを、existing resourcesを利用するnon-persistent logical projectionとして明示した。Persistent domain model/schema/cardinalityは変更しない。
