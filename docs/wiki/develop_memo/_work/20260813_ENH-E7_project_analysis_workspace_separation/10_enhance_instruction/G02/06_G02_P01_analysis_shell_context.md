...
Analysis Workspace shellと明示的current analysis-input contextを作成する。
...
- Analysis Workspace内でProjectを直接切り替えない。
- context selection変更だけでFamily/Stage URLを書き換えない。
- 不足context resourceを暗黙作成しない。
...
|---|---|---|
| focused product test | `uv run pytest -q tests/product/test_enh_e7_g02_p01_analysis_shell_context.py` | PASS |
