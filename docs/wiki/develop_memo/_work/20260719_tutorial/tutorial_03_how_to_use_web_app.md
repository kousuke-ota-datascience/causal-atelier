# ariadne Webアプリ利用チュートリアル

## 1. このチュートリアルで行うこと

起動済みのariadneへ分析可能な単一CSVを登録し、Frontendだけで次の一連の操作を行います。
...
Web MVPはETL製品ではありません。複数raw fileのjoin、集計、時点整合、業務固有の変換等はDatabricksなどの外部ETLへ任せ、ariadneには「1行が1分析単位」になった単一CSVまたはParquetを登録します。
...
推定値は宣言したconsistency、exchangeability、positivity等の仮定の下で解釈します。ariadneは仮定と来歴を固定し診断を提示しますが、因果識別の成立を自動証明しません。
