# G3 Trial 001 Test 007 — full_active_pytest
...
Full active pytest suite を完走して regression を検証する。

## Acceptance Criteria

`uv run pytest -q` が完走し全 test が PASS する。
...
テスト指示書 §14 の fail-fast 規則により高コスト full pytest を未実行。
