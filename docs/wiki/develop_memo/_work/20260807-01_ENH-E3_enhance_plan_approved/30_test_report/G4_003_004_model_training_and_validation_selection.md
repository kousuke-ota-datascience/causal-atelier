# G4 Trial 003 Test 004 — model_training_and_validation_selection
...
分類・回帰training、validation selection、seed決定性、model Artifact/Result契約を検証する。

## Acceptance Criteria

classification/regression model、task compatibility、TRAIN/VALIDATIONのみ、TEST不使用、deterministic seed、model Artifact、selected hyperparameters/validation metric、外部model objectなし。
...
- /tmp/g4_003_002_004_predictive_training.log; logistic/linear registry model、selected hyperparameters、validation ROC_AUC valueの直接assertion。
...
分類・回帰とTrial 002で不足したmetadata直接検証が成功したためPASS。
