...
| 1.3 | 2026-08-03 | 実装に合わせ、実行管理ドメインの用語を`Run`から`Execution`へ改称。API `/runs`→`/executions`、識別子`run_id`→`execution_id`、`Stage Run`→`Stage Execution`、要件ID `FR-RUN-*`→`FR-EXE-*`、状態遷移節を更新。CLIとWeb APIが単一の`execution_id`を共有し、Plannerが独自ID採番を行わないことを明文化(CLI execution_id統合)。`execution_mode`値の`DRY_RUN`/`VALIDATE_ONLY`/`RUN`、status `RUNNING`、Stage実行アダプタ`Stage Runner`、外部システムのRun IDは従来名を維持 |
...
5. CLIを廃止せず、Web APIとCLIを同一Application Serviceへの別Adapterとして共存させ、両Adapterが単一の`Execution`集約と`execution_id`を共有する。
...
| C. Web Orchestration | Dataset Version、Semantics、Execution、Graph、Designの選択・来歴管理 | v1.3 Web MVPの中心機能とする |
...
Discovery Executionで生成された探索結果から、利用者が推論に使用するGraph Versionを選択し、そのGraph VersionをInference Executionの入力にできる。
...
推定値だけでなく、Dataset Version、Feature Semantics、Discovery Execution、Graph Version、Causal Design、adjustment set、仮定、診断へ遡れる。
...
| 領域 | 現行責務 |
|---|---|
| `application` | 実行計画、Execution管理、validation、stage実行契約 |
...
| `interfaces.api` | Project、Dataset、Configuration、Execution、Result、Artifact等のHTTP API |
| `interfaces.cli` | discovery、inference、統合pipelineのCLI |
| `workers` | 非同期Execution、Artifact materialization、結果projection |
...
- Execution、Stage Execution、Attempt、Event
...
| Result導線 | ExecutionからResult IDを直接取得しにくい | ExecutionごとのResult一覧またはExecution詳細へのResult参照を追加する |
...
Complete Journey ETL、8 table Registry、既存feature builderは、回帰テスト、デモデータ生成、CLI利用のために維持すること。v1.3対応を理由に削除、名称変更、引数変更、既定動作変更を行ってはならない。変更が必要な場合は、既存呼出しを維持する互換Adapterまたはdefaultを設けること。
...
| Actor | 主な責務 |
|---|---|
| Viewer | Dataset概要、Execution、Graph、結果、診断、レポート、来歴を閲覧する |
...
- Analysis-ready Tableを直接利用するDiscovery Execution
- Analysis-ready Tableを直接利用するEdge Weight / Treatment Effect Inference Execution
...
- Execution状態、event、retry、cancel
- Project単位認可と監査
...
- Project、Dataset、Configuration、Graph、Execution等のResource管理
- 不変version管理
- Execution受付とExecution Plan生成
- validation要求
- Execution状態管理
- 認証・認可
...
- Dataset、Execution、Saved Graph、Artifact等をProject削除と同時に物理削除せず、各Resourceのretention policyに従って保持すること。
- MVP Frontendから復元できないことと、保持Resourceが即時物理削除されないことを確認画面へ明示すること。
- 削除成功後は実行中のFrontend pollingを停止し、Project一覧へ遷移すること。

### FR-PRJ-002

Dataset、Configuration、Experiment、Execution、Graph、Artifact、ResultはProjectに所属すること。
...
Project詳細画面から、Dataset、Discovery、Saved Graph、Causal Design、Inference、Execution履歴へ遷移できること。
...
- Dataset数
- Discovery Execution数
- 保存済みGraph数
- Inference Execution数
- 直近Executionと状態
- 推奨される次の操作
...
外部Datasetを参照する場合、`latest`等の可変参照をExecutionの不変入力として使用してはならない。Execution受付時までにsnapshot versionを解決すること。
...
MVPの通常UIでは、DiscoveryまたはInference Executionのたびに利用者へlocal pathやDataset Registry YAMLの指定を要求しないこと。workerはDataset Versionのmetadataから必要な実行時registryまたはinput snapshotを生成すること。
...
Feature Semantics VersionはDRAFT、VALID、PUBLISHED、DEPRECATEDの状態を持てること。Execution modeが`RUN`の場合、原則としてPUBLISHED Versionを要求すること。
...
Executionは使用したConfiguration Version IDとcontent hashをExecution PlanおよびManifestへ記録すること。
...
同一Discovery Executionで複数algorithmを実行できること。
...
Discovery Executionは最低限次をArtifactとして登録すること。
...
optional dependencyが不足するalgorithmは、Execution全体を不明な状態にせず、実行前validation errorまたはalgorithm単位の明確なUNSUPPORTED/SKIPPEDとして記録すること。
...
Inference Executionは、入力に使用したGraph Version IDとcontent hashをExecution PlanおよびManifestへ記録すること。

### FR-SCG-009

Graph Versionが参照するDataset VersionまたはFeature SemanticsとInference入力が不整合の場合、Execution前にerrorとすること。
...
Causal Designの宣言とExecutionで指定されたtreatment、outcome、estimandが不一致の場合、MVPではerrorとすること。
...
入力Dataset Version、Feature Semantics Version、Saved Graph Versionのnode/column互換性をExecution前に検証すること。
...
## 7.11. Execution管理・Execution Plan
...
Execution受付APIは分析完了を待たず、Execution IDと初期statusを返すこと。
...
Execution PlanはExecution受付後に不変とすること。

### FR-EXE-008

同一Project・同一Idempotency-Keyの重複Execution作成を防止すること。

### FR-EXE-009

Execution、Stage Execution、Attemptを分離し、再試行履歴を上書きしないこと。

### FR-EXE-010

利用者はExecutionのcancelを要求できること。cancelはbest effortであることを明示すること。

### FR-EXE-011

ExecutionとStage Executionのprogress、warning、error、Artifact生成をeventとして取得できること。

### FR-EXE-012

Execution詳細またはExecution Result APIから、生成されたDiscovery Result、Edge Weight Result、Treatment Effect ResultのIDを取得できること。

### FR-EXE-013

Web API経由のExecutionとCLI経由のExecutionは、同一の`Execution`集約と単一の`execution_id`を共有すること。CLIとWebでID体系を分岐させてはならない。

### FR-EXE-014

Execution Planを生成するPlannerは独自の実行IDを採番してはならず、Execution受付時に確定した`execution_id`を受け取って使用すること。CLIからの起動時も、外部から`execution_id`を指定でき、未指定の場合に限り受付層で一意な`execution_id`を発行すること。

### FR-EXE-015

`execution_id`はExecution Plan、Execution詳細、Stage Execution、Attempt、Manifest、Artifact lineageの全体で一貫して使用し、Web/CLIのいずれからでも同一`execution_id`で来歴を追跡できること。
...
画面上のすべての結果から、生成元Execution、Stage Execution、Artifact、Configuration Version、Dataset Version、Saved Graph Versionへ遡れること。

### FR-RES-006 Result discoverability

利用者はResult IDを手入力しなくても、Project、ExecutionまたはExperimentから結果へ到達できること。
...
利用者はExperimentを作成し、複数Executionを所属させられること。
...
Experiment内のExecutionをDataset、algorithm、Graph Version、estimand、method、resultで比較できること。
...
Endpoint名は実装設計で変更してよいが、Dataset Version一覧、ExecutionからResultへの導線、Saved Graphの永続化を欠落させてはならない。
...
本節は、v1.3をもとにコードを変更する際の必須制約である。本節の「既存」は、v1.3実装着手時点の`main`ブランチで動作しているinterface、class、testを指す。
...
既存引数で起動した場合、v1.3対応前と同じConfigured Feature Build経路を使用すること。
...
Webの新規ExecutionはANALYSIS_READY相当modeを明示すること。既存CLI/API requestがmodeを持たない場合は、従来のConfigured Feature Build挙動を維持すること。入力table数、filename、Dataset Kindだけを根拠にmodeを暗黙切替してはならない。
...
同一Execution条件から再実行要求を作成できること。ただし新しいExecution IDを発行すること。
...
外部Datasetを使用する場合、Executionには解決済みsnapshot identifierを記録すること。
...
- Execution受付APIは分析完了を待たない。
- uploadはstreaming処理する。
- preview/profileで全量をAPI processへloadしない。
- workerを水平増設できる。
- ExecutionごとにCPU、memory、timeout上限を設定できる。
...
- 任意Python実行をMVPで禁止する。
- Execution、download、設定publish、Graph publish、権限変更、削除を監査する。
...
- API request、Execution、Stage Execution、Attemptをcorrelation IDで関連付ける。
- JSON構造化logを出力する。
- queue length、wait time、Execution success/failure、stage duration、worker heartbeat、Artifact errorをmetric化する。
...
- 長時間Executionでは状態、stage、開始時刻、最新eventを表示する。
...
4. Feature Semanticsを定義・publishする
   |
5. 複数algorithmでDiscovery Executionを実行する
   |
6. graphを比較する
   |
...
| Project詳細 | 指標、最近のExecution、中心フロー、次の操作 |
...
| Discovery Execution詳細 | status、event、Artifact、warning |
...
したがって、通常Web UIに「前処理Execution」画面が存在しないことと、プログラム内部に前処理classが存在し利用されることは矛盾しない。
...
Complete Journey固有の複数tableまたはFeature Buildを使わず、Analysis-readyな単一tableの選択列にDiscovery Executionを実行できる。
...
利用者が選択したalgorithm結果を、source Execution、Dataset Version、Feature Semantics Version、algorithm、edge hashを持つ不変Graph Versionとしてサーバーへ保存・publishできる。
...
Saved Graph Versionを選択してInference Executionを作成し、ExecutionのExecution PlanとManifestから同Graph Versionへ遡れる。
...
利用者はResult IDを手入力せず、ProjectまたはExecution詳細からDiscovery ResultとInference Resultへ移動できる。

### AC-017 Traceability

画面上の推定値からDataset Version、Feature Semantics Version、Discovery Execution、Saved Graph Version、Causal Design Version、Inference Execution、Artifact、code versionへ遡れる。
...
同一Project・同一Idempotency-KeyのExecution作成要求でExecutionが重複しない。
...
v1.3変更前に成功していたDiscovery、Inference、統合PipelineのCLI commandを同じ必須引数で実行し、Configured Feature Build経路のExecution Planまたは結果を取得できる。
...
ANALYSIS_READY Executionでも、設定された欠損値処理、encoding、標準化、constant列除外、collinearity checkが実行され、そのresolved設定と除外列をArtifactまたはManifestから確認できる。

### AC-026 Web/CLI responsibility boundary

通常Web UIではComplete Journeyの複数file、Registry YAML、ETL Executionを指定せず中心フローを完遂できる。同時に、既存CLIからComplete Journey ETLとFeature Buildを引き続き実行できる。
...
- Execution、Stage Execution、Attempt、Eventの永続化
- dry-run / validate-only / run
...
上記は「再利用できれば望ましい」項目ではなく、v1.3実装中の後方互換制約である。Frontendから呼ばれないことを理由に削除してはならない。
...
#### P0-005 Execution Result link

ExecutionからDiscovery Result、Edge Weight Result、Treatment Effect Resultへ到達するprojection/APIを追加する。
...
- Complete Journeyの回帰テストを維持し、v1.3変更後も実行する。
...
| コンポーネント | v1.3で行うこと | v1.3で行ってはならないこと |
|---|---|---|
...
| Execution schema/planner | 解決済みinput modeを追加し、Plan/Manifestへ記録する | mode未指定の既存requestを勝手にANALYSIS_READYへ変更する |
...
1. 新しい通常Web UIはANALYSIS_READY相当modeを明示してExecutionを作成する。
...
6. resolved modeをExecution Plan、Execution detail、Manifestへ含める。
...
13. Execution Result APIのresponse schema
...
- `src/ariadne/interfaces/api/routers/executions.py`
