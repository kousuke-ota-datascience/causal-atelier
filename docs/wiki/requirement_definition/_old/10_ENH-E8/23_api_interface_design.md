...
本書は、Ariadneを構成する**interface contract**を、実装内部ではなく利用者・component間の境界として定義する。

本書で確定する事項:

- Browser Navigation Context / route contract
- Web API endpoint、request / response DTO、resource boundary
- Authentication / AuthorizationおよびProject境界
- Worker / Runtime内部interface
- CLI / Python Library interface
- validation / error contract
- idempotency contract
- contract versioning
- interface間で引き渡すidentifier、state、payloadの責務境界

各interface contractでは、該当する範囲で次を明示する。

1. interfaceの目的とauthority
2. endpoint / route / command / callable identity
3. input / request contract
4. output / response contract
5. authentication / authorizationまたはscope boundary
6. state / lifecycle上のprecondition
7. validation / error semantics
8. idempotency / versioning semantics
9. 他interfaceへ引き渡すcontextと、引き渡してはならないstate

本書はDB Entityの属性定義、ORM mapping、class内部構造、algorithm実装詳細を定義しない。それらは`21_logical_data_design.md`、`30_detailed_design.md`またはScientific Core側の詳細contractをauthorityとする。

`Navigation Context`はWeb Appのbrowser interface contractであり、`Web API Endpoint`と同じ種類の設計項目ではない。このため本書では、interface種別を上位階層に置き、Browser / Web API / Runtime / CLIを同一粒度で整理する。

## 2. Interface構成と責務境界

本書のinterface architectureは次のとおりである。

```text
Ariadne Interface Contracts
├─ Browser Navigation Interface
│  └─ route / Navigation Context / browser restoration
├─ Web API Interface
│  ├─ Project / Research Context
│  ├─ Dataset / Analysis View
│  ├─ Analysis Specification
│  ├─ Family-specific analytical interface
│  ├─ Execution
│  ├─ Result / Comparison / Lineage
│  ├─ Annotation / Artifact
│  └─ Frontend Support API
├─ Worker / Runtime Internal Interface
├─ CLI / Library Interface
└─ Cross-cutting Contract Policy
   ├─ Error Model
   ├─ Idempotency
   ├─ Contract Versioning
   └─ Forbidden Interface
...
| Interface種別 | 主な利用者 | Authority / 責務 | 非責務 |
| --- | --- | --- | --- |
| Browser Navigation | Web App / Browser | Project/Analysis route、Navigation Context、deep link、reload/Back/Forward | Domain Resource persistence、Execution lifecycle |
| Web API | Web App / external client | Project-scoped resource/query/command contract | Browser history、UI-local navigation state |
| Worker / Runtime Internal | Worker / Application runtime | Execution claim、lease、Stage runtime、Runner boundary | Public browser/API route |
| CLI / Library | headless user / backend use case | headless scientific execution | Browser navigation metadataへの依存 |
| Cross-cutting policy | 上記すべて | error、idempotency、versioning、禁止境界 | 個別domain operationの意味 |
...
#### 3.2.1 Current request identity
...
#### 3.2.3 Project authorization coverage
...
#### 3.3.2 Read-only presentation metadata

Navigation metadata等のpresentation-only responseはpersistent Domain Resource responseと区別する。presentation metadataを返すこと自体によって、Analysis Specification / Execution Plan / Execution / StageExecution等のscientific/runtime resourceを生成・更新してはならない。

Analytical Navigation Metadataのcanonical endpoint、schema、catalog authorityは`5.10.2 Analytical Navigation Metadata`および`5.10.3 Catalog source of truth`で定義する。

## 4. Browser Navigation Interface

Browser Navigation Interfaceは、Web App内で**どのProject / surface / Analysis Family / Navigation Stageを表示しているか**を表すinterfaceである。Navigation Contextは正本Domain Resourceではなく、URL path、必要なclient/workspace state、既存Resource identityのcompositionとして扱う。

### 4.1 Navigation Contextの責務

Navigation Contextの責務は次に限定する。

- Project Management surfaceのroute identity
- Analysis WorkspaceのProject / Family / Navigation Stage identity
- resource deep link identity
- direct link / reload / Back / Forwardで復元可能なbrowser state
- legacy analytical entryからcanonical routeへの一方向normalization

Navigation ContextへAnalysis SpecificationやExecution条件を複製しない。分析条件・scientific stateは正本Resource/APIから取得する。

Analysis Contextは専用backend resourceではない。Current Project / Active Research Context / Dataset Version / Analysis Viewを、既存Project / Context / Dataset / Analysis View interfaceとclient/workspace stateからcompositionする。

### 4.2 Project Navigation contract
...
Project routeはProject Managementのnavigation authorityであり、Analysis Family / Stage stateを所有しない。

### 4.3 Analysis Navigation contract
...
Analysis WorkspaceのCurrent Projectはrouteの`project_id`をauthorityとしread-onlyとする。Family切替時のdefault Stageは`GET /api/v1/navigation/analysis`の`default_stage_id`をauthorityとする。

Navigation Stageはpresentation/application stateであり、runtime `StageType` / `StageExecution`とは別contractである。

### 4.4 Browser restoration / compatibility semantics
...
- Dataset Version変更によりAnalysis Viewがincompatibleになった場合、Analysis View selectionを解除する。Family/Stage routeは書き換えない。
- context不足を補うために架空のResearch Context / Dataset Version / Analysis Viewを生成しない。

### 4.5 Presentation state vocabulary
...
このvocabularyはbrowser/presentation stateであり、Execution / StageExecution lifecycle enumの代替ではない。

## 5. Web API Interface

### 5.1 Project / Research Context API
...
### 5.3 Analysis Specification API

#### 5.3.1 Common envelope
...
#### 5.3.2 Analysis Specification resource operations
...
#### 5.3.4 Predictive specification contract
...
#### 5.6.1 Predictive subgroup evaluation response contract
...
#### 5.7.1 Canonical/common Execution Web API
...
#### 5.7.2 Canonical Execution response
...
#### 5.7.3 Predictive workflow interface
...
#### 5.7.4 Execution Plan / runtime Stage
...
### 5.8 Result / Comparison / Lineage API

#### 5.8.1 Canonical Result API
...
#### 5.8.2 Project-scoped Product Closure
...
### 5.9 Annotation / Artifact API

#### 5.9.1 Canonical/simple Annotation
...
#### 5.9.4 Findings / Model Management
...
### 5.10 Frontend Support API

#### 5.10.1 Operation Availability
...
##### 5.10.1.1 Canonical operation key set
...
##### 5.10.1.2 resource_type × operation structural support
...
##### 5.10.1.5 Scientific/domain prerequisite authority
...
##### 5.10.1.6 Unknown resource / unsupported operation semantics
...
#### 5.10.2 Analytical Navigation Metadata
...
#### 5.10.3 Catalog source of truth
...
### 6.6 Runtime state observation / event policy
...
## 8. Cross-cutting Contract Policy
...
Error contractに関する追加原則:

- 科学的負結果をtechnical HTTP errorへ変換しない。Result/response内のscientific status / warningとして表現する。
- Navigation catalog/route validation failureをruntime Execution failureへ変換しない。
- Project境界を越えるresource existenceをerror detailで漏洩しない。
...
#### 8.2.1 Retry-safe Artifact materialization
...
本章はcurrent contract本文の代替ではなく、Enhancementごとの差分サマリを記録する。現在有効なinterface contractは`§1`〜`§9`をauthorityとする。

### 20.2 ENH-E2 Golden Path Interface Expansion

ENH-E2では、Project論理削除、Discovery Analysis Spec、Graph Versionの`designated_outcome_node` / allowed action projection、Graph Edit Draft、Graph Candidate / Comparison、Inference Outcome整合性validation、および対応error codeを追加した。これらのcurrent contractは後続Enhancementで有効なものを本文へ統合済みであり、本節では差分そのものを再定義しない。

### 20.3 ENH-E3 Versioned Workspace / Unified Analysis Interface Expansion

ENH-E3では、versioned Research Context / Analysis View / Analysis Specificationを中心に、Exploratory・Causal・PredictiveのFamily interface、Execution Plan / Execution、Result / Comparison / Lineage、Annotation / Artifact、Operation Availability、Worker interfaceを統合的に拡張した。後続Enhancementで変更されたauthentication、runtime、navigation等のcontractは本文のcurrent definitionをauthorityとし、本節ではENH-E3時点の過去contractを再掲しない。
